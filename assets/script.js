...
class Node {
    constructor(x, y, color, size, _name) {
        ...
        this.maxRepelDistance = canvas.width/2 //400; 
        this.repelForceFactor = 0.4; 
        this.conForceFactor = 2; 
    ...
    drawNode() {
        ...
        ctx.beginPath();
        if(this.marked) {
            ctx.arc(this.x, this.y, this.size+5*size_modifier, 0, 2 * Math.PI);
            ctx.fillStyle = darkenHex(this.color,0.4);
            ctx.fill();
        }
        ctx.beginPath();
    ...
    emit() {
        ...
        this.marked = true;
        setTimeout(() => {this.marked = false;},600)
    }
...
// Array of colors
function createNodesCircle(numNodes) {
...
size_modifier = parseFloat(canvas.dataset.scale)
// Start the animation with all nodes
animate(nodes);

if (canvas.dataset.version == "pure_circle") {
    createNodesCircle(parseInt(canvas.dataset.amountnodes))
}

if (canvas.dataset.physics == "true") {
    var run_animated = true
    handleMouseEvents(nodes);
}

if (canvas.dataset.pressemit == "true") {
...
if (canvas.dataset.randominp == "true") {

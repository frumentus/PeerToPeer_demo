...
class InfoPackage {
    ...
    drawInfo() {
        if (this.runningInfo) {
            ...
            ctx.beginPath();
            ctx.arc(xpos, ypos, this.size, 0, 2 * Math.PI);
...
class Node {
    ...
    notify(fromnode,info_package) {
        console.log(this._name+" was notfied ")
        this.inforate = 0.5;
        if(!this.infostorage.includes(info_package.id)) {
            this.infostorage.push(info_package.id)
            this.propagate(info_package)
        }
    }   

    propagate(info_package) {
    ...
    emit() {
        id_package_count += 1
        const info_package = {"id":id_package_count,"content":"new Block","color":this.color}
        this.infostorage.push(info_package.id)
        this.propagate(info_package)
    }
...
for (let i = 0; i < numNodes; i++) {
    ...
    const node = addNode(x, y, color, 20, "Node " + (i + 1));
    nodes.push(node);
}
